...
export const meta: MetaFunction = () => [
  { title: "Create Account | Preships Console" },
  {
    name: "description",
    content: "Create a Preships Console account with email/password, Google, or GitHub.",
  },
];
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md items-center px-6 py-12">
      <div className="w-full rounded-2xl border border-border bg-panel p-6 shadow-lg shadow-black/20">
        ...
        <p className="mt-2 text-sm text-text-muted">Start using Preships Console for billing and workspace controls.</p>

        <Form action="/api/auth/sign-up/email" method="post" className="mt-6 space-y-4">
        ...
        <p className="mt-5 text-sm text-text-muted">
          ...
          <Link to={`/login?redirectTo=${encodeURIComponent(redirectTo)}`} className="text-accent hover:underline">
            Sign in
          </Link>
        </p>
      </div>
