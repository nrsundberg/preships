...
import { getFreeTierUsagePageStub } from "~/lib/usage-stubs.server";
...
function formatCompactInt(value: number) {
  return new Intl.NumberFormat(undefined, { maximumFractionDigits: 0, notation: "compact" }).format(value);
}

export async function loader(_args: LoaderFunctionArgs) {
  return getFreeTierUsagePageStub({ now: new Date() });
}
    ...
    <main className="space-y-6 rounded-xl border border-border bg-panel p-6">
      <header className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-text-muted">
            Free-tier quota window: <span className="font-medium text-text-primary">{data.period.label}</span>
            <span className="text-text-muted"> ({formatIsoDateShort(data.period.startIso)} - {formatIsoDateShort(data.period.endIso)})</span>
          </p>
        </div>
        <div className="rounded-lg border border-border bg-bg px-3 py-2 text-sm">
          ...
          <p className="mt-1 font-medium text-text-primary">{formatIsoDateTime(data.period.resetAtIso)}</p>
        </div>
      </header>
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        <article className="rounded-xl border border-border bg-panel-soft p-5 lg:col-span-1">
          ...
          <p className="mt-2 text-sm text-text-muted">
            Runs are billed by the tokens they generate. This view scaffolds counters + quota tracking.
          </p>
          ...
          <div className="mt-5 space-y-4">
            ...
            <div className="rounded-lg border border-border bg-bg p-3">
              <p className="text-xs uppercase tracking-wide text-text-muted">Trend (placeholder)</p>
              <p className="mt-1 text-sm text-text-primary">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-text-muted sm:grid-cols-3">
                {data.cli.trend.points.slice(-6).map((p) => (
                  <div key={p.label} className="rounded border border-border bg-panel-soft px-2 py-2">
                    <p className="truncate text-[11px]">{p.label}</p>
                    <p className="mt-1 font-medium text-text-primary">{formatCompactInt(p.runs)} runs</p>
        ...
        <article className="rounded-xl border border-border bg-panel-soft p-5 lg:col-span-2">
          ...
          <p className="mt-2 text-sm text-text-muted">
            Top models by token consumption (stubbed for UI scaffolding).
          </p>
          ...
          <div className="mt-4 rounded-lg border border-border bg-bg p-3">
            ...
            <p className="mt-1 text-sm text-text-muted">
              Model mix and token efficiency breakdown will replace this summary once usage telemetry is wired up.
            </p>
          </div>
        </article>
      </section>
        ...
        <article className="rounded-xl border border-border bg-panel-soft p-5 lg:col-span-3">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-3">
            <div className="rounded-lg border border-border bg-bg p-4">
              ...
              <p className="mt-2 text-2xl font-semibold text-text-primary">{nf.format(data.cli.counters.remainingRuns)}</p>
              <p className="mt-1 text-sm text-text-muted">of {nf.format(data.quota.monthlyRuns)} in {data.period.label}</p>
            </div>
            <div className="rounded-lg border border-border bg-bg p-4">
              <p className="text-xs uppercase tracking-wide text-text-muted">Remaining tokens</p>
              <p className="mt-2 text-2xl font-semibold text-text-primary">{nf.format(data.cli.counters.remainingTokens)}</p>
              <p className="mt-1 text-sm text-text-muted">of {nf.format(data.quota.monthlyTokens)} in {data.period.label}</p>
            </div>
            <div className="rounded-lg border border-border bg-bg p-4">
              ...
              <p className="mt-1 text-sm text-text-muted">
                Reset at {formatIsoDateTime(data.period.resetAtIso)} (stub)
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="text-xs text-text-muted">
              Period boundaries and counters are loader-backed scaffolding for free-tier quota tracking.
            </div>
          </div>
        </article>
      </section>
